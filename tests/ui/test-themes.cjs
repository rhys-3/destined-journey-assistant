...
source=source.slice(0,start)+`console.log(await require('./theme-tests.cjs')(cdp,evaluate));\n`+source.slice(end);
source=source.replaceAll('.ui-review/preview.html','.ui-review/theme-preview.html').replace("path.resolve('.ui-review/chrome-profile')","path.resolve('.ui-review/chrome-theme-profile')");
