module.exports=async(cdp,evaluate)=>{
 ...
 assert(await evaluate(`!ui.shadow.textContent.includes('旅程')&&!ui.shadow.querySelector('.eyebrow')&&ui.shadow.querySelector('#destined-title').textContent.includes('命定·黄昏 Prime')&&!getComputedStyle(ui.shadow.querySelector('.tabs'),'::before').content.includes('旅程')`));
 results.push('未设定时默认透明度1%，标题保持精简中文布局');
