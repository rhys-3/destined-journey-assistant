...
const mock=`window.getWorldbookNames=async()=>[];window.getGlobalWorldbookNames=()=>[];window.rebindGlobalWorldbooks=async()=>{};window.getLastMessageId=()=>-1;window.getChatMessages=()=>[];window.insertOrAssignVariables=(v)=>{vars={...vars,...structuredClone(v)}};window.data=${JSON.stringify(preset)};window.stored=structuredClone(data);window.vars={managed_values_version:1,style_structure_version:1};window.errors=[];window.addEventListener('error',e=>errors.push(e.message));window.addEventListener('unhandledrejection',e=>errors.push(String(e.reason)));
...
const browser=spawn(process.env.CHROME_PATH || (process.platform==='win32'?'C:/Program Files/Google/Chrome/Application/chrome.exe':'/usr/bin/google-chrome'),['--headless=new','--disable-gpu','--no-first-run','--no-default-browser-check','--remote-debugging-port=0','--remote-allow-origins=*','--user-data-dir='+path.resolve('.ui-review/chrome-profile'),'about:blank'],{windowsHide:true,stdio:['ignore','ignore','pipe']});
let ws;
try{
const endpoint=awaitPromise();
function awaitPromise(){return new Promise((resolve,reject)=>{browser.once('error',reject);let log='';browser.stderr.on('data',v=>{log+=v;const m=log.match(/DevTools listening on (ws:\/\/[^\s]+)/);if(m)resolve(m[1]);});setTimeout(()=>reject(Error('Chrome startup timeout')),12000).unref();});}
...
await ui.deleteUserStyle(created.id);await ui.settle();ui.state.editorUnlocked=false;check('自建文风删除',!data.prompts.some(p=>p.id===created.id));
