...
.dj-dialog{background:var(--panel);color:var(--ink);border:1px solid var(--line);border-radius:16px;padding:20px;width:min(720px,100%);max-height:100%;min-height:0;display:flex;flex-direction:column;overflow:hidden;box-sizing:border-box}
.dj-dialog>h3,.dj-dialog-actions{flex:none}.dj-dialog-body{min-height:0;overflow:auto;overscroll-behavior:contain;overflow-wrap:anywhere}
...
.mobile-layout .dj-dialog-backdrop{padding:8px}.mobile-layout .dj-dialog{padding:14px;border-radius:12px}.mobile-layout .dj-dialog input,.mobile-layout .dj-dialog select,.mobile-layout .dj-dialog textarea{font-size:16px}.mobile-layout .dj-dialog-actions{gap:8px;margin-top:12px}.mobile-layout .dj-dialog-actions button{flex:1;padding:10px 8px;min-width:0}
`;
export function createDialogs({ getRoot, open }) {
  ...
  function show({title='总结',message='',value,rows=12,choices,cancelValue=null,readOnly=false,recordWrite=false,fields,validate}) {
    ...
    return new Promise(resolve=>{
      ...
      const content=doc.createElement('div');content.className='dj-dialog-body';content.append(body);dialog.append(heading,content); let input;
      if(value!==undefined){input=doc.createElement('textarea');input.value=value;input.rows=rows;input.readOnly=readOnly;input.setAttribute('aria-label',title+'内容');content.append(input);}
      ...
      function fitViewport(){
        ...
        const inset=side=>Math.max(0,parseFloat(style.getPropertyValue('--tt-inset-'+side))||0);
        backdrop.style.inset='auto';
        backdrop.style.left=((viewport?.offsetLeft||0)+inset('left'))+'px';
        backdrop.style.top=((viewport?.offsetTop||0)+inset('top'))+'px';
        backdrop.style.width=Math.max(1,(viewport?.width||win.innerWidth)-inset('left')-inset('right'))+'px';
        backdrop.style.height=Math.max(1,(viewport?.height||win.innerHeight)-inset('top')-inset('bottom'))+'px';
      }
      function finish(result){if(finished)return;finished=true;win.removeEventListener('resize',fitViewport);viewport?.removeEventListener('resize',fitViewport);viewport?.removeEventListener('scroll',fitViewport);backdrop.remove();pending.delete(cancel);off();previous?.focus?.({preventScroll:true});resolve(result);}
      ...
      root.querySelector('.destined-root').append(backdrop);fitViewport();
      win.addEventListener('resize',fitViewport);viewport?.addEventListener('resize',fitViewport);viewport?.addEventListener('scroll',fitViewport);
      if(root.querySelector('.destined-root').classList.contains('mobile-layout')){dialog.tabIndex=-1;dialog.focus({preventScroll:true});}
      else (input??controls.values().next().value??actions.firstChild).focus({preventScroll:true});
    });
  }
  return {
