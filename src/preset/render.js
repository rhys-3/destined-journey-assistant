...
import { SEARCH_FIELDS, searchFields, searchPlaceholder, filterSearchPrompts } from './search.js';
...
export function createRender(ctx) {
  ...
  function renderAdvancedTab() {
    const selectedFields = searchFields(ctx.state);
    ...
    const prompts = filterSearchPrompts(filtered, ctx.state);
    return `${renderSectionHeader('预设条目', `按酒馆发送列表排列 · ${used.length} 项，未加入 ${unused.length} 项`)}
      ...
      <div class="entry-search-fields" role="group" aria-label="搜索范围，可多选">
        <span class="search-field-label">搜索范围 <small>可多选，至少一项</small></span>
        <div class="chips">${SEARCH_FIELDS.map(([key, label]) => choiceButton('search-field', key, label, selectedFields.includes(key), selectedFields.length === 1 && selectedFields.includes(key))).join('')}</div>
      </div>
      <label class="search"><span aria-hidden="true">⌕</span><input aria-label="搜索预设条目" type="search" data-action="search" value="${ctx.escapeHtml(ctx.state.search)}" placeholder="${ctx.escapeHtml(searchPlaceholder(ctx.state))}"></label>
      <p class="search-hint">空格分隔多个关键词，每个词都需匹配；可分别出现在选中的范围内。</p>
