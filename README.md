...
```text
https://cdn.jsdelivr.net/gh/rhys-3/destined-journey-assistant@v3.9.0/dist/destined-journey-assistant.js
```
...
默认采用自定义参数：累计 30 楼触发、至少保留最近 10 楼、每批目标 20 楼。也可手选“已开启摘要”推荐的 50/10 或“未开启摘要”推荐的 20/5；助手不检测摘要开关。自动总结和“立即总结”按正文量均衡分批，楼层目标允许约 20% 浮动，并保持完整回复；指定楼层则按所选范围一次生成。默认串行，可选并发与并发数。界面显示当前聊天的实际计划、各批数量和楼层编号。累计 15 条普通总结时，将最早连续的 10 条合成分段大总结。
